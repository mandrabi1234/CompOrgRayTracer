...
int nsurf = 5;
// Represent sphere surfaces as an array
...
/*----Function: tracing a ray through the optical system----*/
...
int trace(RAY *in, RAY *out, Surface *surf){
    ...
    for (i=0; i<3; i++) out->start[i] += start * in->rayDirection[i]; // calculating the point of intersection
...
/*----Function: printing all calculated vector data----*/
...
/*----Function: calculating a vector's dot product----*/
...
/*----Function: normalizing a vector's length to a given input value----*/
